...
```bash
java -jar greenVC-0.1.8.jar bam2var --in data/HG01500.IBS.exome.MT.bam --out resultfolder  
...
```bash
java -jar greenVC-0.1.8.jar haplocheck --in variants.txt --out haplogrepinput.hsd   --VAF 0.05 
...
```bash
java -jar greenVC-0.1.8.jar haplocheck-mtDNA-Server --in heteroplasmies.txt --out haplogrepinput.hsd  --VAF 0.05 
...
```bash
java -jar greenVC-0.1.8.jar lofreq --in inputfile.vcf --out haplogrepinput.hsd 
```
